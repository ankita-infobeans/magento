...
$process = new fixDownloadableDates();
$process->run();

class fixDownloadableDates {
	...
	const FILE_FIXES = "csvs/ecodes_pdf_fixes_07232012.csv";
...
private $dbHost = "10.30.2.172";
	...
	function fixPremiumOrders()
	{
		...
		$totalcount = count($this->orderArray);
		$count = 0;
		...
		foreach($this->orderArray as $order)
		{
			...
			foreach($items as $product)
			{
				...
				//echo $startdate;
				$q = "UPDATE `sales_flat_order_item` SET `created_at`='".$time."' WHERE `item_id`='".$entity_id."'";
				//echo "\n".$q."\n";
			...
			$diff = time() - $time2;
			$time2 = time();
			$total = $time2-$originaltime;
			echo (" $count of $totalcount Took ".$diff." seconds.");
