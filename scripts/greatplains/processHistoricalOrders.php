...
class processHistoricalOrders {
	...
	const FILE_DOWNLOADABLE = "csvs/ecodes_premium_fixes_07232012.csv";
	//const FILE_DOWNLOADABLE = "fixed.csv";
	const FILE_PREMIUM = "csvs/ecodes_pdf_fixes_07232012.csv";
	...
	*/
	...
	function createOrder($_order) {
		...
		*/
		...
		$paymenttype = "checkmo";
		if (strtolower($_order['status']) == "credit_card") {
			$paymenttype = "ccsave";
		}
		if (strtolower($_order['status']) == "purchase_order") {
		...
		*/
		...
		foreach($items as $item)
		{
			...
			{
				$this->setupDownloadable($item,$product,$order);
			}//else{
  ...
  `created_at` timestamp NOT NULL DEFAULT '0000-00-00 00:00:00',
	...
	function setupDownloadable($item,$product,$downloadable)
	{
