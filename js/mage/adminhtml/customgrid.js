...
varienGrid.prototype = {
    ...
    doExportCoupon : function(varName) {
       
        if($(this.containerId+'_'+varName)){
            if (varName == 'export') {
               if( $(this.containerId+'_export_coupon').visible() ){
                    var text = $(this.containerId+'_export_coupon')[$(this.containerId+'_export_coupon').selectedIndex].text;
                    $(this.containerId+'_export_coupon').setStyle ({
                       'display': 'none'
                    });
                     $(this.containerId+'_export').setStyle ({
                       'display': ''
                    });
                    var options = $$('select#'+this.containerId+'_export option');
                ...
                if( $(this.containerId+'_export').visible() ){
                    var text = $(this.containerId+'_export')[$(this.containerId+'_export').selectedIndex].text;
                    $(this.containerId+'_export').setStyle ({
                       'display': 'none'
                    });
                     $(this.containerId+'_export_coupon').setStyle ({
                    ...
                    var options = $$('select#'+this.containerId+'_export_coupon option');
                    var len = options.length;
                    for (var i = 0; i < len; i++) {
                        if (options[i].text == text) {
                            options[i].selected = true;
            ...
            var exportUrl = $(this.containerId+'_'+varName).value;
            if(this.massaction && this.massaction.checkedString) {
                exportUrl = this._addVarToUrl(exportUrl, this.massaction.formFieldNameInternal, this.massaction.checkedString);
            }
            location.href = exportUrl;
