    ...
    extends Mage_Eav_Model_Convert_Parser_Abstract
{
    ...
    public function unparse()
    {
        ...
        if($this->getVar('date_from') != "" && $this->getVar('date_to') != "" && $this->getVar('filter_by_order_status') != "" ) {
            ...
            //->load();
            $query = "SELECT `main_table`.`entity_id` FROM `sales_flat_order` AS `main_table` WHERE (created_at >= '".$date_from."' AND created_at <= '".$date_to."') AND (status = '".$this->getVar('filter_by_order_status')."')";
        } else if($this->getVar('date_from') != "" && $this->getVar('date_to') != "" && $this->getVar('store') != "0") {
            $date_from = $this->getVar('date_from'). " 00:00:00";
            $date_to = $this->getVar('date_to'). " 23:59:59";
            //$orders =  Mage::getResourceModel('sales/order_collection')
            //->addAttributeToSelect('*')
            //->addAttributeToFilter ( 'store_id' , $this->getVar('store'))
            //->addAttributeToFilter ( 'created_at' , array( "from" => $date_from, "to" => $date_to, "datetime" => true ))
            //->load();
            $query = "SELECT `main_table`.`entity_id` FROM `sales_flat_order` AS `main_table` WHERE (store_id = '".$this->getVar('store')."') AND (created_at >= '".$date_from."' AND created_at <= '".$date_to."')";
            ...
            //->addAttributeToFilter ( 'created_at' , array( "from" => $date_from, "to" => $date_to, "datetime" => true ))
            //->load();
            $query = "SELECT `main_table`.`entity_id` FROM `sales_flat_order` AS `main_table` WHERE (created_at >= '".$date_from."' AND created_at <= '".$date_to."')";
        } else if($this->getVar('filter_by_order_status') != "" ) {
            //$orders =  Mage::getResourceModel('sales/order_collection')
            //->addAttributeToSelect('*')
            //->addFieldToFilter ( 'status' , array( "eq" => $this->getVar('filter_by_order_status') ))
            //->load();
            $query = "SELECT `main_table`.`entity_id` FROM `sales_flat_order` AS `main_table` WHERE (status = '".$this->getVar('filter_by_order_status')."') order by `entity_id` desc LIMIT 0 , 8;";
        } else if($this->getVar('store') != "0" ) {
            //$orders =  Mage::getResourceModel('sales/order_collection')
            //->addAttributeToSelect('*')
            //->addAttributeToFilter ( 'store_id' , $this->getVar('store'))
            //->load();
            $query = "SELECT `main_table`.`entity_id` FROM `sales_flat_order` AS `main_table` WHERE (store_id = '".$this->getVar('store')."')";
        } else {
            //$orders =  Mage::getResourceModel('sales/order_collection')->addAttributeToSelect('*')->load();
            $query = " SELECT `main_table`.`entity_id` FROM `sales_flat_order` AS `main_table`";
            #$entityIds = $this->getData();
        }

        $recordlimit = $this->getVar('recordlimit');
        Mage::log('query'. $query.' limit ' .$recordlimit, null, 'eorders.log');
        $orders = $conn->fetchAll($query.' limit 0, ' .$recordlimit);
        $systemFields = array();
		...
		set_time_limit(0);		
				if ($overallcount < $recordlimit) {
                                Mage::log('entityId'. $overallcount.'---'.$entityId['entity_id'], null, 'eorders.log'); $i++;
				$order = Mage::getModel('sales/order')->load($entityId['entity_id']);
				#print_r($order);
