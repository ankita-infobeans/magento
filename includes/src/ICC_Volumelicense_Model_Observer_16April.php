    ...
    public function createChildOrder($observer) {
       ...
       foreach($order->getAllItems() as $item) {
		...
		$purchasingAgentDetail = Mage::getResourceModel('customer/customer_collection')->addFieldToFilter('email', $purchasingAgentEmail)->addAttributeToSelect('firstname')->addAttributeToSelect('lastname')->getFirstItem();
                $emails = json_decode($order->getVolumeUsers(),true);
                Mage::log($order->getVolumeUsers(),null,'testae.log');
                Mage::log($emails,null,'testae.log');
                foreach($emails as $data) {
		    foreach($data as $key=>$value) {
			$explodeValues = explode(',',$value);
			Mage::log(print_r($explodeValues,1),null,'email.log',true);
		    }
                }
                $customer = Mage::getModel("customer/customer"); 
                $customer->setWebsiteId(Mage::app()->getWebsite()->getId());
                
                foreach($explodeValues as $multipleEmail) {
		    //if($multipleEmail != $purchasingAgentEmail) {
			$customer = Mage::getResourceModel('customer/customer_collection')->addFieldToFilter('email', $multipleEmail)->addAttributeToSelect('firstname')->addAttributeToSelect('lastname')->getFirstItem();
			...
			//echo "<pre>";print_r($item);die("==============___________");
			  ...
			  $this->generateOrder($customer, $item,$multipleEmail,$order);    
			} //die("second");
		    //}
                }
	    }
  ...
  /*  public function associateVolumelicenseInformation($observer) 
    ...
    */
    ...
    public function updateRefundedVolumeLicense($observer){
        ...
        $expire_id =  Mage::app()->getRequest()->getParam('volume_customers');
