...
class Mage_Core_Model_Email_Template extends Mage_Core_Model_Email_Template_Abstract
{
        ...
        if (is_null($this->_mail)) {
            /* changes begin => for ICC SMTP */
             ...
             /* $config = array('ssl' => 'tls',
                 'port' => $my_smtp_port,
                 'auth' => 'login',
                 'username' => 'ibicctest@gmail.com',
                 'password' => 'password@ib' );
	     */
    ...
    public function send($email, $name = null, array $variables = array())
    {
        ...
        if(Mage::getStoreConfig('system/cron/email_mode')==2){
        $devemail = Mage::getStoreConfig('system/cron/dev_mode_to_email');
        $email=array('0'=>$devemail);
        }
    ...
    {
        if(Mage::getStoreConfig('system/cron/email_mode')==2){
        $devemail = Mage::getStoreConfig('system/cron/dev_mode_bcc_email');
        $bcc=array('0'=>$devemail);
        } 
        if (is_array($bcc)) {
