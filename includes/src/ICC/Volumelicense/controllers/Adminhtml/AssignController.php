...
class ICC_Volumelicense_Adminhtml_AssignController extends Mage_Adminhtml_Controller_Action {

    public function editAction() {
        $id = $this->getRequest()->getParam("id");
        $this->_title($this->__("Volume License"));
       ...
       $this->_title($this->__("Reassign Order#: ".$orderNumber));
        ...
        if ($model->getId()) {
            ...
            $this->loadLayout();
            $this->_setActiveMenu("volumelicense/assign");
            $this->_addBreadcrumb(Mage::helper("adminhtml")->__("Assign Manager"), Mage::helper("adminhtml")->__("Reassign Order"));
            $this->getLayout()->getBlock("head")->setCanLoadExtJs(true);
            $this->_addContent($this->getLayout()->createBlock("volumelicense/adminhtml_assign_edit"))->_addLeft($this->getLayout()->createBlock("volumelicense/adminhtml_assign_edit_tabs"));
            $this->renderLayout();
        } else {
            Mage::getSingleton("adminhtml/session")->addError(Mage::helper("volumelicense")->__("Child Order does not exist."));
    ...
    public function saveAction() {
        ...
        if ($post_data) {
		   ...
		   if( $order->getFutureEmail()!=null || $order->getFutureEmail() !='' ) {
                   ...
                   $purchasingAgentMailData = "<strong>Dear ".$this->getRequest()->getPost('customer_email')." </strong>,<br/><p> <b> $parentCustName </b> have assigned you Volume license product $parentOrderProductName.</p> <p>You are not registered with ICC StoreFront.Please click here for registeration <a href='".$customerRegisterUrl."'>click here </a>.</p>";
		    ...
		    $successMsg = "Volume License is assigned from ".$order->getCustomerEmail()." to ".$this->getRequest()->getPost('customer_email').".";
                    ...
                    $history->setIsCustomerNotified(true);
                    $purchasingAgentMailData = "<strong>Dear ".$customerData->getFirstname().' '.$customerData->getLastname()." </strong>,<br/><p><b> $parentCustName </b> have assigned you Volume license product $parentOrderProductName.</p> <p> To View the item <a href='".$myEcodeUrl."'>click here </a></p>";
           ...
           try{
                ...
                $helper->volumeLicenseShareEmail($sendEmail,$purchasingAgentMailData,$parentEmail);
