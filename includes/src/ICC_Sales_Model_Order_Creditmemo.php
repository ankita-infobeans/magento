<?php
class ICC_Sales_Model_Order_Creditmemo extends Mage_Sales_Model_Order_Creditmemo
{
    ...
    public function refund()
    {
        ...
        if ($baseOrderRefund > Mage::app()->getStore()->roundPrice($this->getOrder()->getBaseTotalPaid())) {
            ...
            Mage::throwException(
                Mage::helper('sales')->__('Maximum amount available to refund is %s not %s : %s',
                    $this->getOrder()->formatBasePrice($baseAvailableRefund),$baseOrderRefund ,Mage::app()->getStore()->roundPrice($this->getOrder()->getBaseTotalPaid())
                )
            );
            /*
            Mage::throwException(
